...
class BlogThingy(val blogSettings: BlogSettings, sslContext: SSLContext) : AbstractHttpsThingy(blogSettings.port!!, sslContext) {
    ...
    companion object {
        ...
        private fun embedEntry(entry: BlogEntry): String {
            val page = Page("/de/mel/web/blog/entry.template.html"
                    , Replacer("title") {
...
//        }
        ...
        contextCreator.createContext("/$subUrl/blog.css")
                .withGET()
                .handle { httpExchange, queryMap ->
                    respondText(httpExchange, "/de/mel/web/blog/blog.css")
                }
    ...
    private fun pageWrite(user: String?, pw: String?, id: Long?): Page {
        ...
        return Page("/de/mel/web/blog/write.html", Replacer("pw", pw),
    ...
    private fun pageWithEntries(url: String, entries: List<BlogEntry>): Page? {
        //load template page, with head line and so on
        val page = pageCache.constructOrGet(url, "/de/mel/web/blog/index.html"
    ...
    private fun pageLogin(ex: HttpExchange, id: Long? = null): Page? {
        val url = ex.requestURI.toString()
        if (id == null)
            return pageCache.constructOrGet(url, "/de/mel/web/blog/login.html", Replacer(PARAM_ID, null))
        return pageCache.constructOrGet(url, "/de/mel/web/blog/login.html", Replacer(PARAM_ID, id.toString()))
    }
